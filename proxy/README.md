...
A proxy that saves navigated URLs to [ArchiveStream](https://github.com/mitmproxy/mitmproxy), implemented with [mitmproxy](https://github.com/mitmproxy/mitmproxy).

*✨ Contributed by [Bruno Schroeder (@brunocek)](https://github.com/brunocek/archivestream-proxy)*

---
This project is intended to meet ArchiveStream's ticket 557: [Feature Request: Browser extension to submit either all history or certain URLs to a given ArchiveStream instance](https://github.com/ArchiveStream/ArchiveStream/issues/577).
...
- ArchiveStream is installed and runs.
- User can run `archivestream add <url>` on the machine where this project will run, and have it add the URL to the desired instance of ArchiveStream
- python3 and pipenv are installed (else, on Debian, run `sudo apt-get install python3 pipenv` )

### Debian

1. Clone this repository and have `mitmdump` on the same path (e.g. `/home/user/archivestream-proxy` )

Please follow ( https://docs.mitmproxy.org/stable/overview-installation/ ) to install mitmdump.

```
PATH=/home/user/archivestream-proxy
git clone https://codeberg.org/brunoschroeder/archivestream-proxy.git $PATH
...
2. Edit with vim `config-archivestream-proxy.yaml` and re-define `archivestream-path`
...
4. Edit with vim `archivestream-proxy.service` and re-define `User`, `Group`, and `WorkingDirectory`

5. Run: 

```
sudo ln -sv $PATH/archivestream-proxy.service /etc/systemd/system/archivestream-proxy.servicey
sudo systemctl enable archivestream-proxy
sudo systemctl start archivestream-proxy
sudo systemctl status archivestream-proxy
q

```

6. The **archivestream-proxy** should be up and running on port 8080. Test it by configuring a browser or device:
...
Config file ( `config-archivestream-proxy.yaml` ) holds a parameter for `mode` that can be: **record**, or **archive**.

The reason for two modes is explained in the section bellow **Identifying User HTTP Requests - not trivial**.

On **record mode**, archivestream-proxy will record all the navigation on `record.yaml` file, and the user will need to latter on manually run `archivestream add record.yaml`. The user may edit the file with vim and remove some of the lines ( `dd` ) with URLs not for archiving.

On **archive mode**, archivestream-proxy will run `archivestream add` to each of the identified URLs. Please read section **Identifying User HTTP requests - not trivial** bellow, before using this mode.
