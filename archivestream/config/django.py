...
def setup_django_minimal():
    ...
    # os.environ.setdefault('ARCHIVESTREAM_DATA_DIR', str(CONSTANTS.DATA_DIR))
...
def setup_django(check_db=False, in_memory_db=False) -> None:
    ...
    with Progress(transient=True, expand=True, console=STDERR) as INITIAL_STARTUP_PROGRESS:
        ...
        from archivestream.config.permissions import IS_ROOT, ARCHIVESTREAM_USER, ARCHIVESTREAM_GROUP, SudoPermission
    
        # if running as root, chown the data dir to the archivestream user to make sure it's accessible to the archivestream user
        if IS_ROOT and ARCHIVESTREAM_USER != 0:
            with SudoPermission(uid=0):
                ...
                os.system(f'chown {ARCHIVESTREAM_USER}:{ARCHIVESTREAM_GROUP} "{CONSTANTS.DATA_DIR}" 2>/dev/null')
                os.system(f'chown {ARCHIVESTREAM_USER}:{ARCHIVESTREAM_GROUP} "{CONSTANTS.DATA_DIR}"/* 2>/dev/null')
        ...
        try:
            ...
            if in_memory_db:
                ...
                os.environ.setdefault("ARCHIVESTREAM_DATABASE_NAME", ":memory:")
                django.setup()
            ...
            if check_db:
                ...
                if CONSTANTS.DATA_DIR.stat().st_uid == 0:
                    STDERR.print('[red][X] Error: ArchiveStream DATA_DIR cannot be owned by root![/red]')
                    STDERR.print(f'    {CONSTANTS.DATA_DIR}')
                    STDERR.print()
                    STDERR.print('[violet]Hint:[/violet] Are you running archivestream in the right folder? (and as a non-root user?)')
                    STDERR.print('    cd path/to/your/archive/data')
                    STDERR.print('    archivestream [command]')
                    STDERR.print()
                    raise SystemExit(9)
                ...
                # if archivestream gets imported multiple times, we have to close
                ...
                assert os.access(sql_index_path, os.F_OK), (
                    f'No database file {sql_index_path} found in: {CONSTANTS.DATA_DIR} (Are you in an ArchiveStream collection directory?)')
                ...
                #     logfire.info(f'Started ArchiveStream v{CONSTANTS.VERSION}', argv=sys.argv)
