...
def detect_installed_version(PACKAGE_DIR: Path=PACKAGE_DIR):
    """Autodetect the installed archivestream version by using pip package metadata, pyproject.toml file, or package.json file"""
    try:
        # if in production install, use pip-installed package metadata
        return importlib.metadata.version('archivestream').strip()
    ...
    # raise Exception('Failed to detect installed archivestream version!')
    return 'dev'
...
def get_BUILD_TIME() -> str:
    if IN_DOCKER:
        try:
            # if we're in the archivestream official docker image, /VERSION.txt will contain the build time
...
#     """
#     returns a dictionary containing the ArchiveStream GitHub release info for
...
#     github_releases_api = "https://api.github.com/repos/ArchiveStream/ArchiveStream/releases"
#     response = requests.get(github_releases_api)
#     if response.status_code != 200:
#         stderr(f'[!] Warning: GitHub API call to check for new ArchiveStream version failed! (status={response.status_code})', color='lightyellow', config=config)
#         return None
