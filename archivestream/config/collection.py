...
from archivestream.config.constants import CONSTANTS
...
def load_config_file() -> Optional[benedict]:
    """load the ini-formatted config file from DATA_DIR/Archivestream.conf"""
...
def section_for_key(key: str) -> Any:
    for config_section in archivestream.pm.hook.get_CONFIGS().values():
...
def write_config_file(config: Dict[str, str]) -> benedict:
    ...
    from archivestream.misc.system import atomic_write

    CONFIG_HEADER = (
    """# This is the config file for your ArchiveStream collection.
    #
    # You can add options here manually in INI format, or automatically by running:
    #    archivestream config --set KEY=VALUE
    #
    ...
    #    https://github.com/ArchiveStream/ArchiveStream/wiki/Configuration
    ...
    try:
        ...
        updated_config = {**load_all_config(), **archivestream.pm.hook.get_FLAT_CONFIG()}
...
def load_config(defaults: Dict[str, Any],
    ...
    for key, default in defaults.items():
        try:
            ...
            stderr('        https://github.com/ArchiveStream/ArchiveStream/wiki/Configuration')
            stderr()
            # raise
            # raise SystemExit(2)
