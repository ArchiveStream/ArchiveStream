__package__ = 'archivestream.config'
...
from .permissions import SudoPermission, IS_ROOT, ARCHIVESTREAM_USER

#############################################################################################

PACKAGE_DIR: Path = Path(__file__).resolve().parent.parent    # archivestream source code dir
DATA_DIR: Path = Path(os.getcwd()).resolve()                  # archivestream user data dir
ARCHIVE_DIR: Path = DATA_DIR / 'archive'                      # archivestream snapshot data dir
...
def _get_collection_id(DATA_DIR=DATA_DIR, force_create=False) -> str:
    collection_id_file = DATA_DIR / '.archivestream_id'
    ...
    try:
        ...
        if collection_is_active or force_create:
            ...
            # if we're running as root right now, make sure the collection_id file is owned by the archivestream user
            if IS_ROOT:
                with SudoPermission(uid=0):
                    if ARCHIVESTREAM_USER == 0:
                        os.system(f'chmod 777 "{collection_id_file}"')
                    else:    
                        os.system(f'chown {ARCHIVESTREAM_USER} "{collection_id_file}"')
    except (OSError, FileNotFoundError, PermissionError):
        pass
...
def get_machine_id() -> str:
    ...
    try:
        import machineid
        MACHINE_ID = machineid.hashed_id('archivestream')[:8]
    except Exception:
        try:
            import uuid
            import hashlib
...
def assert_dir_can_contain_unix_sockets(dir_path: Path) -> bool:
    ...
    try:
        ...
        raise Exception(f'ArchiveStream failed to create a test UNIX socket file in {pretty_path(dir_path, color=False)}') from e
...
def create_and_chown_dir(dir_path: Path) -> None:
    with SudoPermission(uid=0, fallback=True):
        ...
        os.system(f'chown {ARCHIVESTREAM_USER} "{dir_path}" 2>/dev/null')
        os.system(f'chown {ARCHIVESTREAM_USER} "{dir_path}"/* 2>/dev/null &')

@cache
def get_or_create_working_tmp_dir(autofix=True, quiet=False):
    from archivestream import CONSTANTS
    from archivestream.config.common import STORAGE_CONFIG
    from archivestream.misc.checks import check_tmp_dir
    ...
    CANDIDATES = [
        ...
        Path('/var/run/archivestream') / get_collection_id(),                     # /var/run/archivestream/abc5d8512
        Path('/tmp') / 'archivestream' / get_collection_id(),                     # /tmp/archivestream/abc5d8512
        Path('~/.tmp/archivestream').expanduser() / get_collection_id(),          # ~/.tmp/archivestream/abc5d8512
        Path(tempfile.gettempdir()) / 'archivestream' / get_collection_id(),      # /var/folders/qy/6tpfrpx100j1t4l312nz683m0000gn/T/archivestream/abc5d8512
        Path(tempfile.gettempdir()) / 'archivestream' / get_collection_id()[:4],  # /var/folders/qy/6tpfrpx100j1t4l312nz683m0000gn/T/archivestream/abc5d
    ...
    if not quiet:
        raise OSError(f'ArchiveStream is unable to find a writable TMP_DIR, tried {CANDIDATES}!')

@cache
def get_or_create_working_lib_dir(autofix=True, quiet=False):
    from archivestream import CONSTANTS
    from archivestream.config.common import STORAGE_CONFIG
    from archivestream.misc.checks import check_lib_dir
    
    # try a few potential directories in order of preference
    CANDIDATES = [
        STORAGE_CONFIG.LIB_DIR,                                                   # <user-specified>
        CONSTANTS.DEFAULT_LIB_DIR,                                                # ./data/lib/arm64-linux-docker
        Path('/usr/local/share/archivestream') / get_collection_id(),                # /usr/local/share/archivestream/abc5
        *([Path('/opt/homebrew/share/archivestream') / get_collection_id()] if os.path.isfile('/opt/homebrew/bin/archivestream') else []),  # /opt/homebrew/share/archivestream/abc5
        Path('~/.local/share/archivestream').expanduser() / get_collection_id(),     # ~/.local/share/archivestream/abc5
    ]
    ...
    if not quiet:
        raise OSError(f'ArchiveStream is unable to find a writable LIB_DIR, tried {CANDIDATES}!')
...
#     - must be scoped by CPU architecture, OS family, and archivestream version
#     - should not be shared with other hosts/archivestream versions
#     - must be writable by any archivestream user
...
#     HOST_DIRS = PlatformDirs(appname='archivestream', appauthor='ArchiveStream', version=detect_installed_version(), opinion=True, ensure_exists=False)
...
#         else:
#             with SudoPermission(uid=ARCHIVESTREAM_USER, fallback=True):
#                 lib_dir = HOST_DIRS.site_data_path
        
#         # Docker: /usr/local/share/archivestream/0.8.5
#         # Ubuntu: /usr/local/share/archivestream/0.8.5
#         # macOS: /Library/Application Support/archivestream
#         try:
#             with SudoPermission(uid=0, fallback=True):
...
#         if IS_ROOT or not dir_is_writable(lib_dir, uid=ARCHIVESTREAM_USER):
#             if IS_ROOT:
#                 # make sure lib dir is owned by the archivestream user, not root
#                 with SudoPermission(uid=0):
#                     if ARCHIVESTREAM_USER == 0:
#                         # print(f'[yellow]:warning:  Waring: Creating SYSTEM_LIB_DIR {lib_dir} with mode 777 so that non-root archivestream users can share it.[/yellow] (caches shared libs used by archivestream for performance)', file=sys.stderr)
#                         os.system(f'chmod -R 777 "{lib_dir}"')
#                     else:
#                         os.system(f'chown {ARCHIVESTREAM_USER}:{ARCHIVESTREAM_GROUP} "{lib_dir}"')
#             else:
#                 raise PermissionError()
#     except (PermissionError, AssertionError):
#         # raise PermissionError(f'SYSTEM_LIB_DIR {lib_dir} is not writable by archivestream user {ARCHIVESTREAM_USER}:{ARCHIVESTREAM_GROUP}')
#         print(f'[red]:cross_mark:  ERROR: SYSTEM_LIB_DIR {lib_dir} is not writable by archivestream user {ARCHIVESTREAM_USER}:{ARCHIVESTREAM_GROUP}[/red]', file=sys.stderr)
...
#     - must be writable by archivestream user & root
#     - must be cleared on every boot / not persisted
#     - must be cleared on every archivestream version upgrade
#     """
#     from .version import detect_installed_version
    
#     HOST_DIRS = PlatformDirs(appname='archivestream', appauthor='ArchiveStream', version=detect_installed_version(), opinion=True, ensure_exists=False)
    
#     # print('DATA_DIR OWNED BY:', ARCHIVESTREAM_USER, ARCHIVESTREAM_GROUP)
...
#             if not dir_is_writable(run_dir, uid=ARCHIVESTREAM_USER):
#                 if IS_ROOT:
#                     with SudoPermission(uid=0, fallback=False):
#                         if ARCHIVESTREAM_USER == 0:
#                             # print(f'[yellow]:warning:  Waring: Creating SYSTEM_TMP_DIR {run_dir} with mode 777 so that non-root archivestream users can access it.[/yellow]', file=sys.stderr)
#                             os.system(f'chmod -R 777 "{run_dir}"')
#                         else:
#                             os.system(f'chown {ARCHIVESTREAM_USER}:{ARCHIVESTREAM_GROUP} "{run_dir}"')
#                 else:
#                     raise PermissionError()
...
#             run_dir = Path(tempfile.gettempdir()).resolve() / 'archivestream' / get_collection_id(DATA_DIR=DATA_DIR)
...
#         if IS_ROOT or not dir_is_writable(run_dir, uid=ARCHIVESTREAM_USER):
#             if IS_ROOT:
#                 with SudoPermission(uid=0):
#                     if ARCHIVESTREAM_USER == 0:
#                         # print(f'[yellow]:warning:  Waring: Creating SYSTEM_TMP_DIR {run_dir} with mode 777 so that non-root archivestream users can access it.[/yellow]', file=sys.stderr)
...
#     except (PermissionError, AssertionError):
#         # raise PermissionError(f'SYSTEM_TMP_DIR {run_dir} is not writable by archivestream user {ARCHIVESTREAM_USER}:{ARCHIVESTREAM_GROUP}')
#         print(f'[red]:cross_mark:  ERROR: SYSTEM_TMP_DIR {run_dir} is not writable by archivestream user {ARCHIVESTREAM_USER}:{ARCHIVESTREAM_GROUP}[/red]', file=sys.stderr)
