...
from archivestream.config.constants import (
    SQL_INDEX_FILENAME,
    JSON_INDEX_FILENAME,
    HTML_INDEX_FILENAME,
)
...
class TestAdd(unittest.TestCase):
    ...
    def test_add_arg_url(self):
        with output_hidden():
            archivestream_add.main(['https://getpocket.com/users/nikisweeting/feed/all'])
    ...
    def test_add_arg_file(self):
        ...
        with output_hidden():
            archivestream_add.main([test_file])
    ...
    def test_add_stdin_url(self):
        with output_hidden():
            archivestream_add.main([], stdin=test_urls)
...
class TestRemove(unittest.TestCase):
    ...
    def test_remove_exact(self):
        with output_hidden():
            archivestream_remove.main(['--yes', '--delete', 'https://example5.com/'])
    ...
    def test_remove_regex(self):
        with output_hidden():
            archivestream_remove.main(['--yes', '--delete', '--filter-type=regex', r'http(s)?:\/\/(.+\.)?(example\d\.com)'])
    ...
    def test_remove_domain(self):
        with output_hidden():
            archivestream_remove.main(['--yes', '--delete', '--filter-type=domain', 'example5.com', 'example6.com'])
    ...
    def test_remove_none(self):
        try:
            with output_hidden(show_failing=False):
                archivestream_remove.main(['--yes', '--delete', 'https://doesntexist.com'])
