...
if len(sys.argv) > 1 and sys.argv[1] == 'setup':
    from rich import print
    print(':warning: [bold red]DEPRECATED[/bold red] `archivestream setup` is deprecated, use `archivestream install` instead')
    sys.argv[1] = 'install'
...
#     """find and import all valid archivestream_<subcommand>.py files in CLI_DIR"""
#     COMMANDS = []
#     for filename in os.listdir(CLI_DIR):
#         if is_cli_module(filename):
#             subcommand = filename.replace('archivestream_', '').replace('.py', '')
#             module = import_module('.archivestream_{}'.format(subcommand), __package__)
...
SUBCOMMAND_MODULES = {
    'help': 'archivestream_help',
    'version': 'archivestream_version' ,
    
    'init': 'archivestream_init',
    'install': 'archivestream_install',
    ##############################################
    'config': 'archivestream_config',
    'add': 'archivestream_add',
    'remove': 'archivestream_remove',
    'update': 'archivestream_update',
    'list': 'archivestream_list',
    'status': 'archivestream_status',
    
    'schedule': 'archivestream_schedule',
    'server': 'archivestream_server',
    'shell': 'archivestream_shell',
    'manage': 'archivestream_manage',

    # 'oneshot': 'archivestream_oneshot',
}
...
is_cli_module = lambda fname: fname.startswith('archivestream_') and fname.endswith('.py')
...
def run_subcommand(subcommand: str,
    ...
    from archivestream.misc.checks import check_migrations
...
def main(args: List[str] | Omitted=OMITTED, stdin: IO | Omitted=OMITTED, pwd: str | None=None) -> None:
    ...
    parser = argparse.ArgumentParser(
        prog=__command__,
        description='ArchiveStream: The self-hosted internet archive',
        add_help=False,
    )
