__package__ = 'archivestream.machine'
...
from archivestream.abid_utils.models import ABIDModel, ABIDField, AutoDateTimeField, ModelWithHealthStats
...
class MachineManager(models.Manager):
    def current(self) -> 'Machine':
        """Get the current machine that ArchiveStream is running on."""
        
        global _CURRENT_MACHINE
        if _CURRENT_MACHINE:
            expires_at = _CURRENT_MACHINE.modified_at + timedelta(seconds=MACHINE_RECHECK_INTERVAL)
            if timezone.now() < expires_at:
                # assume current machine cant change *while archivestream is actively running on it*
...
class InstalledBinaryManager(models.Manager):
    def get_from_db_or_cache(self, binary: Binary) -> 'InstalledBinary':
        ...
        if not binary.abspath or not binary.version or not binary.sha256:
            ...
            binary = archivestream.pm.hook.binary_load(binary=binary, fresh=True)
...
class InstalledBinary(ABIDModel, ModelWithHealthStats):
    ...
    def clean(self, *args, **kwargs) -> None:
        ...
        if not self.binprovider:
            all_known_binproviders = list(abx.as_dict(archivestream.pm.hook.get_BINPROVIDERS()).values())
            binary = archivestream.pm.hook.binary_load(binary=Binary(name=self.name, binproviders=all_known_binproviders), fresh=True)
    ...
    def BINARY(self) -> Binary:
        for binary in abx.as_dict(archivestream.pm.hook.get_BINARIES()).values():
    ...
    def BINPROVIDER(self) -> BinProvider:
        for binprovider in abx.as_dict(archivestream.pm.hook.get_BINPROVIDERS()).values():
    ...
    def load_fresh(self) -> Binary:
        return archivestream.pm.hook.binary_load(binary=self.BINARY, fresh=True)
