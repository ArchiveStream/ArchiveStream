...
from archivestream.config import CONSTANTS, DATA_DIR, VERSION
from archivestream.config.common import SHELL_CONFIG
from archivestream.misc.system import get_dir_size
from archivestream.misc.util import enforce_types
from archivestream.misc.logging import ANSI, stderr
...
def reject_stdin(caller: str, stdin: Optional[IO]=sys.stdin) -> None:
    ...
    if not stdin.isatty():
        ...
        if stdin_raw_text.strip():
            ...
            stderr(f'    Run archivestream "{caller} --help" to see usage and examples.')
            stderr()
            # raise SystemExit(1)
    return None
...
def log_cli_command(subcommand: str, subcommand_args: List[str], stdin: Optional[str | IO], pwd: str='.'):
    ...
    version_msg = '[dark_magenta]\\[{now}][/dark_magenta] [dark_red]ArchiveStream[/dark_red] [dark_goldenrod]v{VERSION}[/dark_goldenrod]: [green4]archivestream [green3]{subcommand}[green2] {args}[/green2]'.format(
...
def log_archiving_paused(num_links: int, idx: int, timestamp: str):
    ...
    print('        archivestream update --resume={}'.format(timestamp))
...
def log_archiving_finished(num_links: int):
    ...
    if Snapshot.objects.count() < 50:
        ...
        print('        archivestream server 0.0.0.0:8000')
...
def log_archive_method_finished(result: "ArchiveResult"):
    ...
    if result.status == 'failed':
        ...
        if os.environ.get('IN_DOCKER') in ('1', 'true', 'True', 'TRUE', 'yes'):
            docker_hints = (
                '  docker run -it -v $PWD/data:/data archivestream/archivestream /bin/bash',
            )
...
def pretty_path(path: Union[Path, str], pwd: Union[Path, str]=DATA_DIR, color: bool=True) -> str:
    """convert paths like .../ArchiveStream/archivestream/../output/abc into output/abc"""
