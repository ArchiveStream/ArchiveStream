...
These are the old types we used to use before ArchiveStream v0.4 (before we switched to Django).
"""

__package__ = 'archivestream.index'
...
from archivestream.config import ARCHIVE_DIR, CONSTANTS

from archivestream.misc.system import get_dir_size
from archivestream.misc.util import ts_to_date_str, parse_date
from archivestream.misc.logging import stderr, ANSI
...
class Link:
    ...
    @property
    def url_hash(self):
        from archivestream.misc.util import hashurl

        return hashurl(self.url)

    @property
    def scheme(self) -> str:
        from archivestream.misc.util import scheme
        return scheme(self.url)

    @property
    def extension(self) -> str:
        from archivestream.misc.util import extension
        return extension(self.url)

    @property
    def domain(self) -> str:
        from archivestream.misc.util import domain
        return domain(self.url)

    @property
    def path(self) -> str:
        from archivestream.misc.util import path
        return path(self.url)

    @property
    def basename(self) -> str:
        from archivestream.misc.util import basename
        return basename(self.url)

    @property
    def base_url(self) -> str:
        from archivestream.misc.util import base_url
    ...
    def is_static(self) -> bool:
        from archivestream.misc.util import is_static_file
