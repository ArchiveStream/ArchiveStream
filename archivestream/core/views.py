...
from archivestream.config import CONSTANTS_CONFIG, DATA_DIR, VERSION
from archivestream.config.common import SHELL_CONFIG, SERVER_CONFIG
from archivestream.misc.util import base_url, htmlencode, ts_to_date_str
from archivestream.misc.serve_static import serve_static_with_byterange_support

from archivestream.logging_util import printable_filesize
from archivestream.search import query_search_index
    ...
    def render_live_index(request, snapshot):
        ...
        context = {
            ...
            **link_info['canonical'],
            ...
            'warc_path': warc_path,
            'SAVE_ARCHIVE_DOT_ORG': archivestream.pm.hook.get_FLAT_CONFIG().SAVE_ARCHIVE_DOT_ORG,
                ...
                return HttpResponse(
                    format_html(
                        (
                            ...
                            f'<code style="user-select: all; color: #333">archivestream update -t timestamp {snapshot.timestamp}</code></pre><br/><br/>'
...
def live_config_list_view(request: HttpRequest, **kwargs) -> TableContext:
    ...
    for section_id, section in reversed(list(CONFIGS.items())):
        for key, field in section.model_fields.items():
            ...
            rows['Default'].append(mark_safe(f'<a href="https://github.com/search?q=repo%3AArchiveStream%2FArchiveStream+path%3Aconfig+{key}&type=code"><code style="text-decoration: underline">{find_config_default(key) or "See here..."}</code></a>'))
            # rows['Documentation'].append(mark_safe(f'Wiki: <a href="https://github.com/ArchiveStream/ArchiveStream/wiki/Configuration#{key.lower()}">{key}</a>'))
...
def live_config_value_view(request: HttpRequest, key: str, **kwargs) -> ItemContext:
    ...
    if key in CONSTANTS_CONFIG:
        section_header = mark_safe(f'[CONSTANTS]   &nbsp; <b><code style="color: lightgray">{key}</code></b> &nbsp; <small>(read-only, hardcoded by ArchiveStream)</small>')
    elif key in FLAT_CONFIG:
        section_header = mark_safe(f'data / ArchiveStream.conf &nbsp; [{find_config_section(key)}]  &nbsp; <b><code style="color: lightgray">{key}</code></b>')
    else:
        ...
        data=[
            {
                ...
                "help_texts": {
                    'Key': mark_safe(f'''
                        <a href="https://github.com/ArchiveStream/ArchiveStream/wiki/Configuration#{key.lower()}">Documentation</a>  &nbsp;
                        <span style="display: {"inline" if aliases else "none"}">
                            Aliases: {", ".join(aliases)}
                        </span>
                    '''),
                    'Type': mark_safe(f'''
                        <a href="https://github.com/search?q=repo%3AArchiveStream%2FArchiveStream+path%3Aconfig+{key}&type=code">
                            See full definition in <code>archivestream/config</code>...
                        </a>
                    '''),
                    'Value': mark_safe(f'''
                        ...
                        Default: &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; 
                        <a href="https://github.com/search?q=repo%3AArchiveStream%2FArchiveStream+path%3Aconfig+{key}&type=code">
                            <code>{find_config_default(key) or '↗️ See in ArchiveStream source code...'}</code>
                        </a>
                        <br/><br/>
                        <p style="display: {"block" if key in FLAT_CONFIG else "none"}">
                            <i>To change this value, edit <code>data/ArchiveStream.conf</code> or run:</i>
                            <br/><br/>
                            <code>archivestream config --set {key}="{
                                val.strip("'")
