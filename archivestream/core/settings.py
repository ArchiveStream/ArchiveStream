...
from archivestream.config import DATA_DIR, PACKAGE_DIR, ARCHIVE_DIR, CONSTANTS  # noqa
from archivestream.config.common import SHELL_CONFIG, SERVER_CONFIG      # noqa
...
################################################################################
### ArchiveStream Plugin Settings
################################################################################

ALL_PLUGINS = archivestream.ALL_PLUGINS
LOADED_PLUGINS = archivestream.LOADED_PLUGINS
...
INSTALLED_APPS = [
    ...
    # Our ArchiveStream-provided apps
    # 'abid_utils',                # handles ABID ID creation, handling, and models
    'config',                    # ArchiveStream config settings (loaded as a plugin, don't need to add it here) 
    ...
    # ArchiveStream plugins
    *abx.as_list(abx.pm.hook.get_INSTALLED_APPS()),  # all plugin django-apps found in archivestream/plugins_* and data/user_plugins,
...
DATABASE_NAME = os.environ.get("ARCHIVESTREAM_DATABASE_NAME", str(CONSTANTS.DATABASE_FILE))
...
# automatically fix case when user sets ALLOWED_HOSTS (e.g. to archivestream.example.com)
# but forgets to add https://archivestream.example.com to CSRF_TRUSTED_ORIGINS
...
IPYTHON_KERNEL_DISPLAY_NAME = 'ArchiveStream Django Shell'
if IS_SHELL:
...
TIME_ZONE = CONSTANTS.TIMEZONE        # django convention is TIME_ZONE, archivestream config uses TIMEZONE, they are equivalent
...
ADMIN_DATA_VIEWS = {
    ...
    "URLS": [
        ...
        {
            "route": "binaries/",
            "view": "archivestream.config.views.binaries_list_view",
            "name": "Dependencies",
            "items": {
                "route": "<str:key>/",
                "view": "archivestream.config.views.binary_detail_view",
                "name": "binary",
            },
        },
        {
            "route": "plugins/",
            "view": "archivestream.config.views.plugins_list_view",
            "name": "Plugins",
            "items": {
                "route": "<str:key>/",
                "view": "archivestream.config.views.plugin_detail_view",
                "name": "plugin",
            },
        },
        {
            "route": "workers/",
            "view": "archivestream.config.views.worker_list_view",
            "name": "Workers",
            "items": {
                "route": "<str:key>/",
                "view": "archivestream.config.views.worker_detail_view",
                "name": "worker",
            },
        },
        {
            "route": "logs/",
            "view": "archivestream.config.views.log_list_view",
            "name": "Logs",
            "items": {
                "route": "<str:key>/",
                "view": "archivestream.config.views.log_detail_view",
                "name": "log",
            },
        },
...
# Must delete archivestream/templates/admin to use because it relies on some things we override
