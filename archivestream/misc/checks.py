__package__ = 'archivestream.misc'
...
# this file is imported by archivestream/__init__.py
# and any imports here will be imported by EVERYTHING else
# so this file should only be used for pure python checks
# that don't need to import other parts of ArchiveStream

# if a check needs to import other parts of ArchiveStream,
...
def check_data_folder() -> None:
    from archivestream import DATA_DIR, ARCHIVE_DIR
    from archivestream.config import CONSTANTS
    from archivestream.config.paths import create_and_chown_dir, get_or_create_working_tmp_dir, get_or_create_working_lib_dir
    
    archive_dir_exists = os.access(ARCHIVE_DIR, os.R_OK) and ARCHIVE_DIR.is_dir()
    if not archive_dir_exists:
        print('[red][X] No archivestream index found in the current directory.[/red]', file=sys.stderr)
        print(f'    {DATA_DIR}', file=sys.stderr)
        print(file=sys.stderr)
        print('    [violet]Hint[/violet]: Are you running archivestream in the right folder?', file=sys.stderr)
        print('        cd path/to/your/archive/folder', file=sys.stderr)
        print('        archivestream [command]', file=sys.stderr)
        print(file=sys.stderr)
        print('    [violet]Hint[/violet]: To create a new archive collection or import existing data in this folder, run:', file=sys.stderr)
        print('        archivestream init', file=sys.stderr)
        raise SystemExit(2)
...
def check_migrations():
    from archivestream import DATA_DIR
    ...
    if pending_migrations and not is_migrating:
        print('[red][X] This collection was created with an older version of ArchiveStream and must be upgraded first.[/red]')
        print(f'    {DATA_DIR}', file=sys.stderr)
        print(file=sys.stderr)
...
def check_io_encoding():
    ...
    #     print('    See https://github.com/ArchiveStream/ArchiveStream/wiki/Troubleshooting#python for help upgrading your Python installation.', file=sys.stderr)
    #     raise SystemExit(2)
...
def check_not_root():
    from archivestream.config.permissions import IS_ROOT, IN_DOCKER
    ...
    if IS_ROOT and not (is_getting_help or is_getting_version or is_installing):
        print('[red][!] ArchiveStream should never be run as root![/red]', file=sys.stderr)
        print('    For more information, see the security overview documentation:', file=sys.stderr)
        print('        https://github.com/ArchiveStream/ArchiveStream/wiki/Security-Overview#do-not-run-as-root', file=sys.stderr)
        ...
        if IN_DOCKER:
            ...
            print('        docker compose run archivestream {attempted_command}', file=sys.stderr)
            print(f'        docker run -it -v $PWD/data:/data archivestream/archivestream {attempted_command}', file=sys.stderr)
            print('        or:', file=sys.stderr)
            print(f'        docker compose exec --user=archivestream archivestream /bin/bash -c "archivestream {attempted_command}"', file=sys.stderr)
            print(f'        docker exec -it --user=archivestream <container id> /bin/bash -c "archivestream {attempted_command}"', file=sys.stderr)
        raise SystemExit(2)


def check_data_dir_permissions():
    from archivestream import DATA_DIR
    from archivestream.misc.logging import STDERR
    from archivestream.config.permissions import ARCHIVESTREAM_USER, ARCHIVESTREAM_GROUP, DEFAULT_PUID, DEFAULT_PGID, IS_ROOT, USER
    ...
    data_owner_doesnt_match = (data_dir_uid != ARCHIVESTREAM_USER and data_dir_gid != ARCHIVESTREAM_GROUP) if not IS_ROOT else False
    data_not_writable = not (os.path.isdir(DATA_DIR) and os.access(DATA_DIR, os.W_OK))
    if data_owned_by_root:
        STDERR.print('\n[yellow]:warning: Warning: ArchiveStream [blue]DATA_DIR[/blue] is currently owned by [red]root[/red], it must be changed before archiving can run![/yellow]')
    elif data_owner_doesnt_match or data_not_writable:
        STDERR.print(f'\n[yellow]:warning: Warning: ArchiveStream [blue]DATA_DIR[/blue] is currently owned by [red]{data_dir_uid}:{data_dir_gid}[/red], but ArchiveStream user is [blue]{ARCHIVESTREAM_USER}:{ARCHIVESTREAM_GROUP}[/blue] ({USER})! (ArchiveStream may not be able to write to the data dir)[/yellow]')
        
    if data_owned_by_root or data_owner_doesnt_match or data_not_writable:
        STDERR.print(f'[violet]Hint:[/violet] Change the current ownership [red]{data_dir_uid}[/red]:{data_dir_gid} (PUID:PGID) to a non-root user & group that will run ArchiveStream, e.g.:')
        ...
        STDERR.print('    [link=https://github.com/ArchiveStream/ArchiveStream#storage-requirements]https://github.com/ArchiveStream/ArchiveStream#storage-requirements[/link]')
        STDERR.print('    [link=https://github.com/ArchiveStream/ArchiveStream/wiki/Security-Overview#permissions]https://github.com/ArchiveStream/ArchiveStream/wiki/Security-Overview#permissions[/link]')
        STDERR.print('    [link=https://github.com/ArchiveStream/ArchiveStream/wiki/Configuration#puid--pgid]https://github.com/ArchiveStream/ArchiveStream/wiki/Configuration#puid--pgid[/link]')
        STDERR.print('    [link=https://github.com/ArchiveStream/ArchiveStream/wiki/Troubleshooting#filesystem-doesnt-support-fsync-eg-network-mounts]https://github.com/ArchiveStream/ArchiveStream/wiki/Troubleshooting#filesystem-doesnt-support-fsync-eg-network-mounts[/link]')
...
def check_tmp_dir(tmp_dir=None, throw=False, quiet=False, must_exist=True):
    from archivestream.config.paths import assert_dir_can_contain_unix_sockets, dir_is_writable, get_or_create_working_tmp_dir
    from archivestream.misc.logging import STDERR
    from archivestream.config.permissions import ARCHIVESTREAM_USER, ARCHIVESTREAM_GROUP
    from archivestream.config.common import STORAGE_CONFIG
    from archivestream.logging_util import pretty_path
    ...
    try:
        ...
        assert tmp_is_valid, f'ArchiveStream user PUID={ARCHIVESTREAM_USER} PGID={ARCHIVESTREAM_GROUP} is unable to write to TMP_DIR={tmp_dir}'            
        assert len(f'file://{socket_file}') <= 96, f'ArchiveStream TMP_DIR={tmp_dir} is too long, dir containing unix socket files must be <90 chars.'
        return True
        ...
        if not quiet:
            ...
            ERROR_TEXT = '\n'.join((
                '',
                f'[red]:cross_mark: ArchiveStream is unable to use TMP_DIR={pretty_path(tmp_dir)}[/red]',
                f'   [yellow]{e}[/yellow]',
                '',
                ...
                f'  - It [red]must[/red] be readable and writable by the ArchiveStream user (PUID={ARCHIVESTREAM_USER}, PGID={ARCHIVESTREAM_GROUP}).',
                ...
                f'      [green]archivestream config --set TMP_DIR={get_or_create_working_tmp_dir(autofix=False, quiet=True) or "/tmp/archivestream"}[/green]',
                '',
            ))
            ...
            STDERR.print()
        if throw:
            raise OSError(f'TMP_DIR={tmp_dir} is invalid, ArchiveStream is unable to use it and the server will fail to start!') from e
    return False
...
def check_lib_dir(lib_dir: Path | None = None, throw=False, quiet=False, must_exist=True):
    ...
    from archivestream.config.permissions import ARCHIVESTREAM_USER, ARCHIVESTREAM_GROUP
    from archivestream.misc.logging import STDERR
    from archivestream.config.paths import dir_is_writable, get_or_create_working_lib_dir
    from archivestream.config.common import STORAGE_CONFIG
    from archivestream.logging_util import pretty_path
    
    lib_dir = lib_dir or STORAGE_CONFIG.LIB_DIR
    
    assert lib_dir == archivestream.pm.hook.get_LIB_DIR(), "lib_dir is not the same as the one in the flat config"
    ...
    try:
        ...
        assert lib_is_valid, f'ArchiveStream user PUID={ARCHIVESTREAM_USER} PGID={ARCHIVESTREAM_GROUP} is unable to write to LIB_DIR={lib_dir}'
        return True
    except Exception as e:
        if not quiet:
            STDERR.print()
            ERROR_TEXT = '\n'.join((
                '',
                f'[red]:cross_mark: ArchiveStream is unable to use LIB_DIR={pretty_path(lib_dir)}[/red]',
                f'   [yellow]{e}[/yellow]',
                '',
                '[blue]Info:[/blue] [grey53]The LIB_DIR is used to store ArchiveStream auto-installed plugin library and binary dependencies.',
                f'  - It [red]must[/red] be readable and writable by the ArchiveStream user (PUID={ARCHIVESTREAM_USER}, PGID={ARCHIVESTREAM_GROUP}).',
                ...
                f'      [green]archivestream config --set LIB_DIR={get_or_create_working_lib_dir(autofix=False, quiet=True) or "/usr/local/share/archivestream"}[/green]',
                '',
            ))
            ...
            STDERR.print()
        if throw:
            raise OSError(f'LIB_DIR={lib_dir} is invalid, ArchiveStream is unable to use it and dependencies will fail to install.') from e
    return False
