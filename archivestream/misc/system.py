...
from archivestream.misc.util import enforce_types, ExtendedEncoder
...
def atomic_write(path: Union[Path, str], contents: Union[dict, str, bytes], overwrite: bool=True) -> None:
    ...
    try:
        ...
        if STORAGE_CONFIG.ENFORCE_ATOMIC_WRITES:
            ...
            print("    but the main folder containing the index.sqlite3 and ArchiveStream.conf files must be on a filesystem that supports FSYNC.")
            raise SystemExit(1)
...
CRON_COMMENT = 'archivestream_schedule'
