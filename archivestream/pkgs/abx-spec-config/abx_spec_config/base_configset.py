...
class BaseConfigSet(BaseSettings):
    """
    This is the base class for an ArchiveStream ConfigSet.
    It handles loading values from schema defaults, ArchiveStream.conf TOML config, and environment variables.

    class WgetConfig(ArchiveStreamBaseConfig):
    ...
    load_from_collection: ClassVar[bool] = True    # read from ./ArchiveStream.conf
    ...
    def settings_customise_sources(
        ...
        """Defines the config precedence order: Schema defaults -> ArchiveStream.conf (TOML) -> Environment variables"""
    ...
    def update_in_place(self, warn=True, persist=False, hint='', **kwargs):
        ...
        if warn:
            fix_scope = 'in ArchiveStream.conf' if persist else 'just for current run'
        ...
        # if persist=True, write config changes to data/ArchiveStream.conf [AUTOFIXES] section
        try:
    ...
    def from_collection(self) -> Dict[ConfigKeyStr, Any]:
        """Get the dictionary of {key: value} config loaded from the collection ArchiveStream.conf"""
    ...
    def to_toml_str(self, defaults=False) -> str:
        ...
        from archivestream.misc.toml_util import CustomTOMLEncoder
