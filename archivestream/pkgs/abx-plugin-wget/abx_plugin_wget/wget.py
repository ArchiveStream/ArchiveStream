...
from archivestream.misc.system import run, chmod_file
from archivestream.misc.util import (
    enforce_types,
    without_fragment,
    without_query,
    path,
...
from archivestream.logging_util import TimedProgress
from archivestream.index.schema import Link, ArchiveResult, ArchiveOutput, ArchiveError
...
def unsafe_wget_output_path(link: Link) -> Optional[str]:
    ...
    # one of the main bottlenecks of ArchiveStream's performance (the output data is often on a slow HDD or network mount).
    ...
    for _ in range(4):
        try:
            if os.access(search_dir, os.R_OK) and search_dir.is_dir():
                ...
                # test this with archivestream add --depth=1 https://getpocket.com/users/nikisweeting/feed/all
    ...
    # this is why ArchiveStream's specializes in REDUNDANTLY saving copies of sites with multiple different tools
    return None
...
def wget_output_path(link: Link, nocache: bool=False) -> Optional[str]:
    ...
    complicated attempt to do this. Here be dragons:
        - https://github.com/ArchiveStream/ArchiveStream/issues/549
        - https://github.com/ArchiveStream/ArchiveStream/issues/1373
    ...
    if output_path:
        ...
        if output_path != safe_path:
            # contains unprintable unicode characters that will break other parts of archivestream
            # better to pretend it doesnt exist and fallback to parent dir than crash archivestream
            output_path = None
