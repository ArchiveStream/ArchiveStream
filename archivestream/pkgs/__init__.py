...
VENDORED_PKGS = [
    'abx',
    # 'pydantic-pkgr',
    # ... everything else in archivestream/pkgs/* comes after ...
]
...
def load_vendored_pkgs():
    """Add archivestream/pkgs to sys.path and import all vendored python packages present within"""
    ...
    for pkg_name in VENDORED_PKGS:
        ...
        try:
            ...
            try:
                ...
                print(f"Failed to import required pkg from sys.path or archivestream/pkgs dir {pkg_name}: {e}", file=sys.stderr)
                sys.exit(1)
