...
from archivestream.config.common import SEARCH_BACKEND_CONFIG
...
class SonicConfig(BaseConfigSet):
    ...
    SONIC_COLLECTION: str   = Field(default='archivestream')
    SONIC_BUCKET: str       = Field(default='archivestream')
    ...
    def validate(self):
        if SEARCH_BACKEND_CONFIG.SEARCH_BACKEND_ENGINE == 'sonic' and SONIC_LIB is None:
            sys.stderr.write('[X] Error: Sonic search backend is enabled but sonic-client lib is not installed. You may need to run: pip install archivestream[sonic]\n')
            # dont hard exit here. in case the user is just running "archivestream version" or "archivestream help", we still want those to work despite broken ldap
