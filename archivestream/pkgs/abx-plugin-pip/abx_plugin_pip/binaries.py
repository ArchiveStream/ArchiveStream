...
def get_archivestream_version():
    try:
        from archivestream import VERSION
        return VERSION
    except Exception:
        return None


class ArchivestreamBinary(Binary):
    name: BinName = 'archivestream'

    binproviders_supported: List[InstanceOf[BinProvider]] = [VENV_PIP_BINPROVIDER, SYS_PIP_BINPROVIDER, apt, brew, env]
    overrides: BinaryOverrides = {
        VENV_PIP_BINPROVIDER.name:  {'packages': [], 'version': get_archivestream_version},
        SYS_PIP_BINPROVIDER.name:   {'packages': [], 'version': get_archivestream_version},
        apt.name:                   {'packages': [], 'version': get_archivestream_version},
        brew.name:                  {'packages': [], 'version': get_archivestream_version},
    }
...
ARCHIVESTREAM_BINARY = ArchivestreamBinary()
