__package__ = 'archivestream.crawls'

import time

import abx
import abx.archivestream.events
import abx.hookimpl
...
# The orchestrator first starts when the archivestream starts, and it stops when archivestream is killed.
...
# When started, the orchestrator runs a single runloop that continues until the archivestream process is killed.
...
def transition_snapshot_to_started(snapshot, config, cwd):
    ...
    snapshot = abx.archivestream.writes.update_snapshot(filter_kwargs=snapshot_to_update, update_kwargs=fields_to_update)
...
def create_queued_archiveresult_on_snapshot(snapshot, config) -> bool | None:
    ...
    existing_results = abx.archivestream.reads.get_archiveresults_from_snapshot(snapshot, extractor='warc')
    ...
    if not has_pending_or_succeeded_results:
        return abx.archivestream.writes.create_archiveresult(snapshot=snapshot, extractor='warc', status='queued', retry_at=time.now())
    return None
...
def transition_archiveresult_to_started(archiveresult, config, cwd):
    ...
    archiveresult = abx.archivestream.writes.update_archiveresult(filter=archiveresult_to_update, update=fields_to_update)
    ...
    with TimedProgress():
        try:
            ...
            archiveresult = abx.archivestream.writes.update_archiveresult(filter_kwargs=archiveresult_to_update, update_kwargs=fields_to_update)
    
    return archiveresult


def transition_archiveresult_to_succeeded(archiveresult, config, cwd):
    output = abx.archivestream.reads.get_archiveresult_output(archiveresult)
    end_ts = time.now()
...
def transition_archiveresult_to_backoff(archiveresult, config, cwd):
    ...
    error = abx.archivestream.reads.get_archiveresult_error(archiveresult, cwd)
    end_ts = time.now()
    output = None
...
def on_archiveresult_created(archiveresult):
    abx.archivestream.exec.exec_archiveresult_extractor(archiveresult)

@abx.hookimpl
def on_archiveresult_updated(archiveresult):
    abx.archivestream.writes.create_snapshots_pending_from_archiveresult_outlinks(archiveresult)




def scheduler_runloop():
    # abx.archivestream.events.on_scheduler_runloop_start(timezone.now(), machine=Machine.objects.get_current_machine())

    while True:
        # abx.archivestream.events.on_scheduler_tick_start(timezone.now(), machine=Machine.objects.get_current_machine())
        ...
        for scheduled_crawl in scheduled_crawls_due:
            try:
                abx.archivestream.events.on_crawl_schedule_tick(scheduled_crawl)
            except Exception as e:
                abx.archivestream.events.on_crawl_schedule_tick_failure(timezone.now(), machine=Machine.objects.get_current_machine(), error=e, schedule=scheduled_crawl)
        
        # abx.archivestream.events.on_scheduler_tick_end(timezone.now(), machine=Machine.objects.get_current_machine(), tasks=scheduled_tasks_due)
        time.sleep(1)
...
def exec_archiveresult_extractor(archiveresult):
    ...
    # abx.archivestream.writes.update_archiveresult_started(archiveresult, start_ts=timezone.now())
    # abx.archivestream.events.on_archiveresult_updated(archiveresult)
    
    # check if it should be skipped
    if not abx.archivestream.reads.get_archiveresult_should_run(archiveresult, config):
        abx.archivestream.writes.update_archiveresult_skipped(archiveresult, status='skipped')
        abx.archivestream.events.on_archiveresult_skipped(archiveresult, config)
        return
    
    # run the extractor method and save the output back to the archiveresult
    try:
        output = abx.archivestream.writes.exec_archiveresult_extractor(archiveresult, config)
        abx.archivestream.writes.update_archiveresult_succeeded(archiveresult, output=output, error=None, end_ts=timezone.now())
    except Exception as e:
        abx.archivestream.writes.update_archiveresult_failed(archiveresult, error=e, end_ts=timezone.now())
    
    # bump the modified time on the archiveresult and Snapshot
    abx.archivestream.events.on_archiveresult_updated(archiveresult)
    abx.archivestream.events.on_snapshot_updated(archiveresult.snapshot)
