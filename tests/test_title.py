...
def test_title_is_htmlencoded_in_index_html(tmp_path, process, disable_extractors_dict):
    """
    https://github.com/ArchiveStream/ArchiveStream/issues/330
    Unencoded content should not be rendered as it facilitates xss injections
    and breaks the layout.
    """
    subprocess.run(['archivestream', 'add', 'http://127.0.0.1:8080/static/title_with_html.com.html'],
...
def test_title_in_meta_title(tmp_path, process, disable_extractors_dict):
    add_process = subprocess.run(["archivestream", "add", "http://127.0.0.1:8080/static/title_with_html.com.html"],
                                   capture_output=True, env=disable_extractors_dict)
...
def test_title_in_meta_og(tmp_path, process, disable_extractors_dict):
    add_process = subprocess.run(["archivestream", "add", "http://127.0.0.1:8080/static/title_og_with_html.com.html"],
                                   capture_output=True, env=disable_extractors_dict)
...
def test_title_malformed(tmp_path, process, disable_extractors_dict):
    add_process = subprocess.run(["archivestream", "add", "http://127.0.0.1:8080/static/malformed.html"],
                                   capture_output=True, env=disable_extractors_dict)
