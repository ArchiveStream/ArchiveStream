...
def test_remove_single_page(tmp_path, process, disable_extractors_dict):
    os.chdir(tmp_path)
    subprocess.run(['archivestream', 'add', 'http://127.0.0.1:8080/static/example.com.html'], capture_output=True, env=disable_extractors_dict)
    remove_process = subprocess.run(['archivestream', 'remove', 'http://127.0.0.1:8080/static/example.com.html', '--yes'], capture_output=True)
...
def test_remove_single_page_filesystem(tmp_path, process, disable_extractors_dict):
    ...
    subprocess.run(['archivestream', 'remove', 'http://127.0.0.1:8080/static/example.com.html', '--yes', '--delete'], capture_output=True)

    assert list((tmp_path / "archive").iterdir()) == []

def test_remove_regex(tmp_path, process, disable_extractors_dict):
    subprocess.run(['archivestream', 'add', 'http://127.0.0.1:8080/static/example.com.html'], capture_output=True, env=disable_extractors_dict)
    subprocess.run(['archivestream', 'add', 'http://127.0.0.1:8080/static/iana.org.html'], capture_output=True, env=disable_extractors_dict)
    assert list((tmp_path / "archive").iterdir()) != []

    subprocess.run(['archivestream', 'remove', '--filter-type=regex', '.*', '--yes', '--delete'], capture_output=True)
...
def test_remove_exact(tmp_path, process, disable_extractors_dict):
    ...
    remove_process = subprocess.run(['archivestream', 'remove', '--filter-type=exact', 'http://127.0.0.1:8080/static/iana.org.html', '--yes', '--delete'], capture_output=True)
...
def test_remove_substr(tmp_path, process, disable_extractors_dict):
    ...
    subprocess.run(['archivestream', 'remove', '--filter-type=substring', 'example.com', '--yes', '--delete'], capture_output=True)
...
def test_remove_domain(tmp_path, process, disable_extractors_dict):
    ...
    remove_process = subprocess.run(['archivestream', 'remove', '--filter-type=domain', '127.0.0.1', '--yes', '--delete'], capture_output=True)
...
def test_remove_tag(tmp_path, process, disable_extractors_dict):
    ...
    remove_process = subprocess.run(['archivestream', 'remove', '--filter-type=tag', 'test-tag', '--yes', '--delete'], capture_output=True)
...
def test_remove_before(tmp_path, process, disable_extractors_dict):
    ...
    subprocess.run(['archivestream', 'remove', '--filter-type=regex', '.*', '--yes', '--delete', '--before', higherts], capture_output=True)
...
def test_remove_after(tmp_path, process, disable_extractors_dict):
    ...
    subprocess.run(['archivestream', 'remove', '--filter-type=regex', '.*', '--yes', '--delete', '--after', lowerts], capture_output=True)
