...
def test_list_json(process, disable_extractors_dict):
    subprocess.run(["archivestream", "add", "http://127.0.0.1:8080/static/example.com.html", "--depth=0"],
                                  capture_output=True, env=disable_extractors_dict)
    list_process = subprocess.run(["archivestream", "list", "--json"], capture_output=True)
...
def test_list_html(process, disable_extractors_dict):
    subprocess.run(["archivestream", "add", "http://127.0.0.1:8080/static/example.com.html", "--depth=0"],
                                  capture_output=True, env=disable_extractors_dict)
    list_process = subprocess.run(["archivestream", "list", "--html"], capture_output=True)
...
def test_list_html_headers(process, disable_extractors_dict):
    subprocess.run(["archivestream", "add", "http://127.0.0.1:8080/static/example.com.html", "--depth=0"],
                                  capture_output=True, env=disable_extractors_dict)
    list_process = subprocess.run(["archivestream", "list", "--html", "--with-headers"], capture_output=True)
    output_html = list_process.stdout.decode("utf-8")
...
def test_list_csv(process, disable_extractors_dict):
    subprocess.run(["archivestream", "add", "http://127.0.0.1:8080/static/example.com.html", "--depth=0"],
                                  capture_output=True, env=disable_extractors_dict)
    list_process = subprocess.run(["archivestream", "list", "--csv", "url"], capture_output=True)
...
def test_list_csv_headers(process, disable_extractors_dict):
    subprocess.run(["archivestream", "add", "http://127.0.0.1:8080/static/example.com.html", "--depth=0"],
                                  capture_output=True, env=disable_extractors_dict)
    list_process = subprocess.run(["archivestream", "list", "--csv", "url", "--with-headers"], capture_output=True)
    output_csv = list_process.stdout.decode("utf-8")
    assert "http://127.0.0.1:8080/static/example.com.html" in output_csv
    assert "url" in output_csv

def test_list_index_with_wrong_flags(process):
    list_process = subprocess.run(["archivestream", "list", "--with-headers"], capture_output=True)
...
def test_link_sort_by_url(process, disable_extractors_dict):
    subprocess.run(["archivestream", "add", "http://127.0.0.1:8080/static/iana.org.html", "--depth=0"],
                                  capture_output=True, env=disable_extractors_dict)
    subprocess.run(["archivestream", "add", "http://127.0.0.1:8080/static/example.com.html", "--depth=0"],
                                  capture_output=True, env=disable_extractors_dict)

    list_process = subprocess.run(["archivestream", "list"], capture_output=True)
    link_list = list_process.stdout.decode("utf-8").split("\n")
    assert "http://127.0.0.1:8080/static/iana.org.html" in link_list[0]

    list_process = subprocess.run(["archivestream", "list", "--sort=url"], capture_output=True)
    link_list = list_process.stdout.decode("utf-8").split("\n")
