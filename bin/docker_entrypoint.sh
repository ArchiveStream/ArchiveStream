...
# This Docker ENTRYPOINT script is called by `docker run archivestream ...` or `docker compose run archivestream ...`.
# It takes a CMD as $* shell arguments and runs it following these setup steps:

# - Set the archivestream user to use the correct PUID & PGID
...
# - Drop down to archivestream user permisisons and execute passed CMD command.
...
export ARCHIVESTREAM_USER="${ARCHIVESTREAM_USER:-archivestream}"
...
if [[ "$PUID" == "0" ]]; then
    echo -e "\n[X] Error: Got PUID=$PUID and PGID=$PGID but ArchiveStream is not allowed to be run as root, please change or unset PUID & PGID and try again." > /dev/stderr
...
# Set archivestream user and group ids to desired PUID/PGID
usermod -o -u "${PUID:-$DETECTED_PUID}" "$ARCHIVESTREAM_USER" > /dev/null 2>&1
groupmod -o -g "${PGID:-$DETECTED_PGID}" "$ARCHIVESTREAM_USER" > /dev/null 2>&1
...
export PUID="$(id -u archivestream)"
export PGID="$(id -g archivestream)"
...
if [[ -d "$DATA_DIR/Documents" || -d "$DATA_DIR/.config" || -d "$DATA_DIR/usr" || -f "$DATA_DIR/.bashrc" || -f "$DATA_DIR/.zshrc" ]]; then
    echo -e "\n[X] ERROR: ArchiveStream was run from inside a home folder"
...
if [[ -d "$DATA_DIR/archive" ]]; then
    if touch "$DATA_DIR/archive/.permissions_test_safe_to_delete" 2>/dev/null; then
     ...
     # the only time this fails is if the host filesystem doesn't allow us to write as root (e.g. some NFS mapall/maproot problems, connection issues, drive dissapeared, etc.)
        echo -e "\n[X] Error: archivestream user (PUID=$PUID) is not able to write to your ./data/archive dir (currently owned by $(stat -c '%u' "$DATA_DIR/archive"):$(stat -c '%g' "$DATA_DIR/archive")." > /dev/stderr
    ...
    # create data directory (and logs, since its the first dir ArchiveStream needs to write to)
...
# force set the ownership of the data dir contents to the archivestream user and group
...
# also chown BROWSERS_DIR because otherwise 'archivestream setup' wont be able to 'playwright install chromium' at runtime
...
export ARCHIVESTREAM_BIN_PATH="$(which archivestream)"

# Drop permissions to run commands as the archivestream user
if [[ "$1" == /* || "$1" == "bash" || "$1" == "sh" || "$1" == "echo" || "$1" == "cat" || "$1" == "whoami" || "$1" == "archivestream" ]]; then
    # handle "docker run archivestream /bin/somecommand --with=some args" by passing args directly to bash -c
    # e.g. "docker run archivestream archivestream init:
    #      "docker run archivestream /venv/bin/ipython3"
    #      "docker run archivestream /bin/bash -c '...'"
    #      "docker run archivestream cat /VERSION.txt"
    ...
    # gosu spawns an ephemeral bash process owned by archivestream user (bash wrapper is needed to load env vars, PATH, and setup terminal TTY)
    ...
    # handle "docker run archivestream add some subcommand --with=args abc" by calling archivestream to run as args as CLI subcommand
    # e.g. "docker run archivestream help"
    #      "docker run archivestream add --depth=1 https://example.com"
    #      "docker run archivestream manage createsupseruser"
    #      "docker run archivestream server 0.0.0.0:8000"
    exec gosu "$PUID" "$ARCHIVESTREAM_BIN_PATH" "$@"
fi
