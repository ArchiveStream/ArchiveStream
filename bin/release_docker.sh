...
# for each tag in TAG_NAMES, add archivestream/archivestream:tag and nikisweeting/archivestream:tag to FULL_TAG_NAMES
for TAG_NAME in "${TAG_NAMES[@]}"; do
    [[ "$TAG_NAME" == "" ]] && continue
    FULL_TAG_NAMES+=("-t archivestream/archivestream:$TAG_NAME")
    FULL_TAG_NAMES+=("-t nikisweeting/archivestream:$TAG_NAME")
    FULL_TAG_NAMES+=("-t ghcr.io/archivestream/archivestream:$TAG_NAME")
done
echo "${FULL_TAG_NAMES[@]}"
...
mkdir -p "$HOME/.cache/docker/archivestream"

# https://docs.docker.com/build/cache/backends/
# shellcheck disable=SC2068
exec docker buildx build \
   --platform "$SELECTED_PLATFORMS" \
   --cache-from type=local,src="$HOME/.cache/docker/archivestream" \
   --cache-to type=local,compression=zstd,mode=min,oci-mediatypes=true,dest="$HOME/.cache/docker/archivestream" \
   --push . ${FULL_TAG_NAMES[@]}   
