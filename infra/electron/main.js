...
// constants
const DATA_DIR = path.join(os.homedir(), 'archivestream')
const BIND_HOST = '0.0.0.0'
const BIND_PORT = '8085'
// const DOCKER_IMAGE = 'ubuntu'
// const DOCKER_CMD = ['uname', '-a']
const DOCKER_IMAGE = 'archivestream/archivestream:latest'
const DOCKER_CMD = ['archivestream', 'server', '--init', `0.0.0.0:${BIND_PORT}`]
...
const updateTray = () => {
    ...
    TRAY.setContextMenu(Menu.buildFromTemplate([
        { label: CONTAINER ? 'Stop ArchiveStream' : 'Start ArchiveStream', click() { 
            if (CONTAINER) {
        ...
        { label: `ArchiveStream is ${container_status}`, enabled: false },
        { type: 'separator' },
        ...
        { label: 'Settings', click() { 
            ...
            shell.openItem(path.join(DATA_DIR, 'ArchiveStream.conf'))
        } },
        { label: 'Update', click() { 
            if (!CONTAINER) {
                startDocker()
            }
            DOCKER.pull('archivestream/archivestream', function (err, stream) {
                stream.pipe(process.stdout)
            });
        } },
        { label: 'Quit', click() {
            if (CONTAINER) {
                console.log('[X] Stopping ArchiveStream docker container...')
                CONTAINER.kill(function() {
                    CONTAINER.remove()
                    CONTAINER = null
                    console.log('[X] Stopping ArchiveStream app gracefully...')
                    app.quit()
                })
            } else {
                console.log('[X] Stopping ArchiveStream app gracefully...')
            ...
            DOCKER.createContainer(
                ...
                function(err, container) {
                    ...
                    container.start(function (err, data) {
                        CONTAINER = container
                        console.log('[√] Started ArchiveStream Docker container')
                        updateTray()
                    })
                },
            )
...
console.log('[+] Starting ArchiveStream...')
...
app.on('ready', () => {
    ...
    console.log('[+] Starting ArchiveStream tray icon...')
    createTray()
    updateTray()
